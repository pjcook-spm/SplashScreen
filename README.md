...
By default, the splash mask uses `Logo.path`. You can replace it with an asset-catalog image via `SplashImageCatalog.logoMaskAssetName`, or override with an explicit vector path via `SplashImageStore(logoPath:)`. The image asset that you use for `logoMaskAssetName` much be a transparent `PNG`.
...
struct DemoApp: App {
    @State private var splashStore = SplashImageStore(
        imageCatalog: SplashImageCatalog(
            ...
            bundle: .main,
            logoMaskAssetName: "BrandLogoMask"
        )
    )
...
- `logoMaskAssetName` in `SplashImageCatalog` to use a bundled image as the mask silhouette

Custom logo path example (takes precedence over catalog `logoMaskAssetName`):
